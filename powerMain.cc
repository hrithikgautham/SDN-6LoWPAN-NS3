...
// 2 nodes is used.
// one is source and the another is destiantion.
// packets are sent for every second.
// nodes is powered by BasicEnergySource.
// recharged by BasicEnergyHarvester.
// wifi radio consumes energy for transmission/reception.
// 
...
void RemainingEnergy (double oldValue, double remainingEnergy)
{
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Current remaining energy on Receiver = " << remainingEnergy << "J");
  //std::cout<<"\n\nend\n\n"<<std::endl;
}

void RemainingSenderEnergy(double oldValue, double remainingEnergy) {
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Current remaining energy on SENDER = " << remainingEnergy << "J");
}
...
{
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Total energy consumed by receiver radio = " << totalEnergy << "J");
  //std::cout<<"\n\nend\n\n"<<std::endl;
}

void TotalSenderEnergy (double oldValue, double totalEnergy) {
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Total energy consumed by SENDER radio = " << totalEnergy << "J");
}
...
{
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Current harvested power of receiver = " << harvestedPower << " W");
  //std::cout<<"\n\nend\n\n"<<std::endl;
}

void HarvestedSenderPower(double oldValue, double harvestedPower) {
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Current harvested power of SENDER = " << harvestedPower << " W");
}
...
{
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Total energy harvested by receiver's harvester = "
                 << TotalEnergyHarvested << " J");
  //std::cout<<"\n\nend\n\n"<<std::endl;
}

void TotalSenderEnergyHarvested (double oldValue, double TotalEnergyHarvested) {
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds ()
                 << "s Total energy harvested by SENDER's harvester = "
                 << TotalEnergyHarvested << " J");
}
...
int main (int argc, char *argv[])
{
  ...
  // initially the energy for each node is set to 1J
  basicSourceHelper.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (1.0));
  // install source
  EnergySourceContainer sources = basicSourceHelper.Install (c);// both nodes charged with energy of 1J
  /* device energy model */
  WifiRadioEnergyModelHelper radioEnergyHelper;
  // configure radio energy model
  radioEnergyHelper.Set ("TxCurrentA", DoubleValue (0.0174));// set the current to default
  radioEnergyHelper.Set ("RxCurrentA", DoubleValue (0.0197));// set the current to default
  // install device model
  DeviceEnergyModelContainer deviceModels = radioEnergyHelper.Install (devices, sources);
  // devices => wifi devices ; sources => nodes installed with energy source
  ...
  static Ipv4Address addressReceiver("10.1.1.2");// receiver address
  TypeId tid = TypeId::LookupByName ("ns3::UdpSocketFactory");
  Ptr<Socket> recvSink = Socket::CreateSocket (networkNodes.Get (1), tid);  // node 1, Destination
  InetSocketAddress local = InetSocketAddress (addressReceiver, 80);
  recvSink->Bind (local);
  recvSink->SetRecvCallback (MakeCallback (&ReceivePacket));// whenever the packet is received by the receiver 'ReceivePacket()' function is called

  Ptr<Socket> source = Socket::CreateSocket (networkNodes.Get (0), tid);    // node 0, Source
  InetSocketAddress remote = InetSocketAddress (addressReceiver, 80);
  source->SetAllowBroadcast (true);
  source->Connect (remote);

  // all traces are connected to node 1 (Destination)
  // energy source
  Ptr<BasicEnergySource> basicSourcePtr = DynamicCast<BasicEnergySource> (sources.Get (1));// receiver
  basicSourcePtr->TraceConnectWithoutContext ("RemainingEnergy", MakeCallback (&RemainingEnergy));// remaining energy on receiver 
  ...
  // RemainingEnergy => TotalEnergy => HarvestedPower => TotalEnergyHarvested ; of receiver


  //** for sender **//
  
  Ptr<BasicEnergySource> basicSenderPtr = DynamicCast<BasicEnergySource> (sources.Get (0));
  basicSenderPtr->TraceConnectWithoutContext ("RemainingEnergy", MakeCallback (&RemainingSenderEnergy));
  Ptr<DeviceEnergyModel> basicSenderRadioModelPtr = basicSenderPtr->FindDeviceEnergyModels ("ns3::WifiRadioEnergyModel").Get (0);
  NS_ASSERT (basicSenderRadioModelPtr != 0);
  basicSenderRadioModelPtr->TraceConnectWithoutContext ("TotalEnergyConsumption", MakeCallback (&TotalSenderEnergy));

//error
  Ptr<BasicEnergyHarvester> basicSenderHarvesterPtr = DynamicCast<BasicEnergyHarvester> (harvesters.Get (0));
  basicSenderHarvesterPtr->TraceConnectWithoutContext ("HarvestedPower", MakeCallback (&HarvestedSenderPower));
  basicSenderHarvesterPtr->TraceConnectWithoutContext ("TotalEnergyHarvested", MakeCallback (&TotalSenderEnergyHarvested));
//error
  // start traffic
  ...
  for (DeviceEnergyModelContainer::Iterator iter = deviceModels.Begin (); iter != deviceModels.End (); iter++)
    {
      ...
      NS_LOG_UNCOND ("End of simulation of node " << " (" << Simulator::Now ().GetSeconds ()
